...
public class SCarteBancaire implements IService<Cartebancaire> {
    ...
    public void ajouter(Cartebancaire cartebancaire) {
        try {
            // Vérifier que la connexion à la base de données est ouverte
            if (con == null || con.isClosed()) {
                System.out.println("La connexion à la base de données est fermée ou non initialisée.");
                return;
            }

            String query = "INSERT INTO carte_bancaire (compte_id, is_frozen, num_carte, cvv, date_exp) VALUES (?, ?, ?, ?, ?)";
    ...
    public void modifier(Cartebancaire cartebancaire) throws SQLException {
        String query = "UPDATE carte_bancaire SET compte_id=?, is_frozen=?, num_carte=?, cvv=?, date_exp=? WHERE id=?";
        ...
        pstmt.setInt(6, cartebancaire.getId());
