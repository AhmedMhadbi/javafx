...
public class FXMLController implements Initializable {
    ...
    private void Login(ActionEvent event) throws SQLException, IOException {
        ...
        Connection connection = utils.DB.getInstance().getConnection();
...
//                mp.play();
            ...
            ps = connection.prepareStatement("SELECT * FROM admin WHERE id = ? and password = ?");
            ps.setString(1, useridtf.getText());
            ps.setString(2, passwordtf.getText());
            rs = ps.executeQuery();
            if (rs.next()) {
                Stage stage = new Stage();
                FXMLLoader loader = new FXMLLoader();
                loader.setLocation(getClass().getResource("/fxml/AdminPageCustomer.fxml"));
                loader.load();
                Parent root = loader.getRoot();
                AdminPageCustomerController apc = loader.getController();
...
//                mp.play();
                ...
                mssg.setText("Wrong Password Or Admin name");
            }
            ps.close();
            rs.close();
        }
