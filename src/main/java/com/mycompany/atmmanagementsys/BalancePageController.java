    ...
    public void WithdrawMoney(ActionEvent event) throws SQLException {
        try {
            if (withdrawamount.getText().isEmpty() || Integer.parseInt(withdrawamount.getText()) < 0) {
                ...
                Connection con = utils.DB.getInstance().getConnection();
                ...
                while (rs.next()) {
                    ...
                    if (NewBalance < 0) {
                        ...
                        ps = con.prepareStatement("UPDATE user SET balance =? WHERE id = '" + UserID + "'");
                ...
                if (rs.next()) {
                    ...
                    if (newbalance < 0) {
                        ...
                        if (rs2.next()) {
                            ...
                            ps2 = con.prepareStatement("UPDATE user SET balance =? WHERE id = '" + receiverid.getText() + "' ");
                            ...
                            PreparedStatement pss = con.prepareStatement("SELECT * FROM user WHERE id =?");
                            pss.setString(1, UserID);
                            ResultSet rss = pss.executeQuery();
                            while (rss.next()) {
                                int addbalance = rss.getInt("Balance") + Integer.parseInt(trnasamount.getText());
                                pss = con.prepareStatement("UPDATE user SET balance = ? WHERE id = '" + UserID + "'");
